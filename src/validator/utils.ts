...
export function getFullPathToValidatorBinary(version?: string): string {
    ...
    if (typeof version === "undefined") {
        ...
        throw new Error(`Couldn't find a TabNineValidator binary (tried the following paths: versions=${versions} ${tried})`);
    } else {
...
function getArchitecture(): string {
    ...
    throw new Error(`Architecture "${process.arch}" is not supported by TabNineValidator`);
}
...
function getTargetAndFileNameByPlatform(): {
    ...
    if (process.platform === "win32") {
        return { target: "pc-windows-gnu", filename: `${validatorBinaryBaseName}.exe` };
    }
    ...
    throw new Error(`Platform "${process.platform}" is not supported by TabNineValidator`);
}
