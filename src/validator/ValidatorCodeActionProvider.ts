...
export class ValidatorCodeActionProvider implements vscode.CodeActionProvider {
	...
	provideCodeActions(document: vscode.TextDocument, range: vscode.Range | vscode.Selection, context: vscode.CodeActionContext, token: vscode.CancellationToken): vscode.CodeAction[] {
		...
		diagnostics.filter(diagnostic => diagnostic.code === TABNINE_DIAGNOSTIC_CODE)
			.forEach(diagnostic => {
	...
	private createCodeAction(document: vscode.TextDocument, diagnostic: TabNineDiagnostic, choice: Completion): vscode.CodeAction {
